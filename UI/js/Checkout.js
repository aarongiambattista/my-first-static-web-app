/**
 * Define eb_checkout class.
 * @class eb_checkout
 * */
var eb_checkout = eb_checkout || {};

/**
 * Control level setting: Site path.
 * @property eb_checkout.SitePath
 * @type {String}
 */
eb_checkout.SitePath = eb_Config.SitePath;

/**
 * Control level setting Template path.
 * @property eb_checkout.TemplatePath
 * @type {String}
 */
eb_checkout.TemplatePath = "html/Checkout.html";

/**
 * The path to the eBusiness SOA layer.
 * @property eb_checkout.ServicePath
 * @type {String}
 */
eb_checkout.ServicePath = eb_Config.ServicePathV1;

/* TODO: Once actual service is ready will update the payPalUrl. */
/**
 * Paypal Url
 * @property eb_checkout.payPalUrl
 * @type {String}
 */
eb_checkout.payPalUrl = "";

/**
 * For Initiating the payment
 * @property eb_checkout.initiatePayment
 * @type {String}
 */
eb_checkout.initiatePayment = "PayPalExpress/InitiatePayment";

/**
 * For Completing the payment
 * @property eb_checkout.completePayment
 * @type {String}
 */
eb_checkout.completePayment = "PayPalExpress/CompletePayment";

/**
 * Order type
 * @property eb_checkout.purchaseOrder
 * @type {String}
 */
eb_checkout.purchaseOrder = "PurchaseOrder";

/**
 * Redirect from checkout page to reviewOrder
 * @property eb_checkout.reviewOrderUrl
 * @type {String}
 */
eb_checkout.reviewOrderUrl = eb_checkout.SitePath + "ReviewOrder.html";

/**
 * Shipping address URL.
 * @property eb_checkout.shippingAddressUrl
 * @type {String}
 */
eb_checkout.shippingAddressUrl = eb_checkout.SitePath + "BillingShippingAddress.html";

/**
 * Order confirmation URL.
 * @property eb_checkout.orderConfirmationUrl
 * @type {String}
 */
eb_checkout.orderConfirmationUrl = eb_checkout.SitePath + "OrderConfirmation.html";

eb_checkout.serviceUrls = {
    'Pay By New Card': eb_checkout.ServicePath + 'ShoppingCarts/Checkout/CreditCard',
    'Pay By Saved Card': eb_checkout.ServicePath + 'ShoppingCarts/Checkout/SavedPayment',
    'Pay By ACH new card': eb_checkout.ServicePath + 'ShoppingCarts/Checkout/ACH',
    'Pay By ACH Tokenizer new card': eb_checkout.ServicePath + 'ShoppingCarts/Checkout/ACHTokenizer',
    'Pay By Bluepay HPP': eb_checkout.ServicePath + 'ShoppingCarts/Checkout/GetRemotePaymentRequest',
    'Pay By GPay': eb_checkout.ServicePath + 'ShoppingCarts/Checkout/GPay',
    'Pay By ApplePay': eb_checkout.ServicePath + 'ShoppingCarts/Checkout/ApplePay',
    'CardPointe Tokenize': eb_checkout.ServicePath + 'CardPointe/ccn/tokenize',
    'CardPointe Tokenize Apple Pay': eb_checkout.ServicePath + 'CardPointe/ccn/tokenizeApplePay'
};

/*Success Response */
eb_checkout.successResponses = {
    'Order placed': 'Your order has been placed successfully.'
};

/* Error messages */
eb_checkout.errorMessages = {
 ...
 * 
 * @property eb_checkout.errorResponses
 * @type {Object}
 * */
eb_checkout.errorResponses = {
 ...
 * @property eb_checkout.defaultErrorMessage
 * @type {String}
 * */
eb_checkout.defaultErrorMessage = 'Sorry, there was an unexpected error. If the problem persists, please contact customer support for further assistance.';
 ...
 * */
eb_checkout.render = function (options) {
    ...
    if (!options.templatePath) {
        var finalPath = eb_checkout.SitePath + eb_checkout.TemplatePath;
 ...
 */
eb_checkout.model = function (options) {
    var _that = this;
    ...
    eb_checkout.domElement(_that.domElement);
    ...
    if (options.payPalUrl) {
        eb_checkout.payPalUrl = options.payPalUrl;
    }

    /* Pass change shipping address URL */
    if (options.reviewOrderUrl) {
        eb_checkout.reviewOrderUrl = options.reviewOrderUrl;
    }

    /* Pass change shipping address URL */
    if (options.orderConfirmationUrl) {
        eb_checkout.orderConfirmationUrl = options.orderConfirmationUrl;
    }
    ...
    _that.paypalPlaceOrder = function () {
        if (_that.shoppingCart) {
            var cardData = {};
            cardData.parameter = eb_checkout.initiatePayment;
            /*window.open(eb_checkout.payPalUrl, "mywindow", "status=1,toolbar=1");*/
            cardData.data = {
                CallingURL: eb_checkout.payPalUrl
            };
            _that.shoppingCart.checkout(cardData).done(function (result) {
                /* need to get PayerID and Token */
                var payPalData = {};
                payPalData.parameter = eb_checkout.completePayment;
                payPalData.data = {
                    PayerID: "W8BUCCG6PEFVQ",
                    Token: "EC-581846091G803745X"
                };
                _that.shoppingCart.checkout(payPalData).done(function (resultPaypal) {
                    console.info(eb_checkout.successResponses['Order placed']);
                    if (eb_checkout.orderConfirmationUrl) {
                        window.location.assign(eb_checkout.orderConfirmationUrl);
                    }
                    else {
    ...
    _that.billMeLaterPlaceOrder = function (data) {
        if (_that.shoppingCart) {
            ...
            var cardData = {};
            cardData.parameter = eb_checkout.purchaseOrder;
            cardData.data = {
    ...
    _that.placeOrder = function (cardData) {
        ...
        _that.shoppingCart.checkout(cardData).done(function (data) {
            ...
            _that.showLoader(0);
            if (eb_checkout.orderConfirmationUrl) {
                window.location.assign(eb_checkout.orderConfirmationUrl + "?" + encodeURIComponent("orderId") + "=" + encodeURIComponent(data.id));
            }
            else {
                console.error("Proceed to checkout URL is required.");
            }
            ...
            if (xhr && typeof xhr.responseJSON !== 'undefined')
                _that.errorMessage(eb_Config.getErrorMessageForControl(xhr.responseJSON, eb_checkout));
            else
                _that.errorMessage(eb_checkout.defaultErrorMessage);
        });
    };

    /* Redirect to review order page. */
    _that.backToReviewOrder = function () {
        if (eb_checkout.reviewOrderUrl) {
            window.location.assign(eb_checkout.reviewOrderUrl);
        }
        else {
    ...
    _that.backToAddress = function () {
        if (eb_checkout.shippingAddressUrl) {
            window.location.assign(eb_checkout.shippingAddressUrl);
        }
        else {
    ...
    _that.payBySavedCard = function (cardInfo) {
        ...
        var serviceURL = eb_checkout.serviceUrls['Pay By Saved Card'];
    ...
    _that.payByBluepayHPP = function () {
        ...
        var serviceURL = eb_checkout.serviceUrls['Pay By Bluepay HPP'];
    ...
    _that.payByiFrameTokenizer = function () {
        ...
        eb_tokenizer.loadTokenizerModal().done(function (data) {
            ...
            //EB-2194 Implementing ACH Tokenizer
            if (data.isACH) {
                cardDetails = {
                ...
                serviceURL = eb_checkout.serviceUrls['Pay By ACH Tokenizer new card'];
            }
            else {
                cardDetails = {
                    cardNumber: token,
                ...
                serviceURL = eb_checkout.serviceUrls['Pay By New Card'];
            }
            _that.showLoader(1);
            eb_Config.retrieveCSRFTokens().always(function (headers) {
                eBusinessJQObject.ajax({
    ...
    _that.isValidPayment = function () {
        if (eb_paymentSummaryDetails.live.subTotalWithoutCurrencySymbol() > 0) {
            return true;
        }
        else {
            return false;
    ...
    _that.tokenizeGPayData = function (gPayTokenData) {
        ...
        var serviceURL = eb_checkout.serviceUrls['CardPointe Tokenize'];
        var postData = {
    ...
    _that.payByGPay = function (data) {
        ...
        var serviceURL = eb_checkout.serviceUrls['Pay By GPay'];
    ...
    _that.tokenizeApplePayData = function (applePayTokenData) {
        ...
        var serviceURL = eb_checkout.serviceUrls['CardPointe Tokenize Apple Pay'];
        var postData = {
    ...
    _that.payByApplePay = function (data) {
        ...
        var serviceURL = eb_checkout.serviceUrls['Pay By ApplePay'];
    ...
    _that.bluepayHPPPostResponseUrl = eb_checkout.ServicePath + 'ShoppingCarts/Checkout/ProcessRemotePaymentResponse';
    ...
    _that.handleBluepayHPPPostResponseSuccess = function (data) {
        ...
        _that.showLoader(0);
        if (eb_checkout.orderConfirmationUrl) {
            window.location.assign(eb_checkout.orderConfirmationUrl + "?" + encodeURIComponent("orderId") + "=" + encodeURIComponent(data.OrderId));
        }
        else {
            console.error("Proceed to checkout URL is required.");
        }
    ...
    _that.payByNewCard = function (cardInfo) {
        if (!isValidCreditCard(cardInfo.cardNumber())) {
            ...
            cardInfo.errorMessage(eb_checkout.errorMessages['Invalid card number']);
            return;
        }

        if (cardInfo.isDateExpired(eb_paymentControl.monthConstants[cardInfo.selectedMonth()], cardInfo.selectedYear())) {
            cardInfo.showError(1);
            cardInfo.errorMessage(eb_checkout.errorMessages['Card date expired']);
            return;
        }
        ...
        var serviceURL = eb_checkout.serviceUrls['Pay By New Card'];
    ...
    _that.payByACHCard = function (cardInfo) {
        ...
        var serviceURL = eb_checkout.serviceUrls['Pay By ACH new card'];
...
/**
* Page DOM element.
* @method eb_checkout.domElement
* @param {object} domElement current DOM element.
* */
eb_checkout.domElement = function (domElement) {
    var self = this;
...
eBusinessJQObject(eb_checkout.domElement).ajaxError(function (event, xhr, settings) {
    if (xhr && typeof xhr.responseJSON !== 'undefined' && (xhr.responseJSON.type === eb_Config.serviceErrorType['Security Requirement Failed'] || xhr.responseJSON.type === eb_Config.serviceErrorType['Http Response']))
        eb_Config.getErrorMessageForControl(xhr.responseJSON, eb_checkout);
});