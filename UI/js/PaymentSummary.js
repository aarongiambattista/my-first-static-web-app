 ...
 * @class eb_paymentSummaryDetails
 * */
var eb_paymentSummaryDetails = eb_paymentSummaryDetails || {};

/**
 * Control level setting: Site path.
 * @property eb_paymentSummaryDetails.SitePath
 * @type {String}
 */
eb_paymentSummaryDetails.SitePath = eb_Config.SitePath;

/**
 * Control level setting Template path.
 * @property eb_paymentSummaryDetails.TemplatePath
 * @type {String}
 */
eb_paymentSummaryDetails.TemplatePath = "html/PaymentSummary.html";

/**
 * The path to the eBusiness SOA layer.
 * @property eb_paymentSummaryDetails.ServicePath
 * @type {String}
 */
eb_paymentSummaryDetails.ServicePath = eb_Config.ServicePathV1;

/**
 * Coupon Name service to get the Id
 * @property eb_paymentSummaryDetails.cName
 * @type {Number}
 */
eb_paymentSummaryDetails.cName = eb_paymentSummaryDetails.ServicePath + "Coupon";

/**
 * Proceed to checkout URL if user has not passed from outside.
 * @property eb_paymentSummaryDetails.proceedToCheckoutUrl
 * @type {String}
 */
eb_paymentSummaryDetails.proceedToCheckoutUrl = eb_paymentSummaryDetails.SitePath + "BillingShippingAddress.html";

/**
 * Proceed to checkout URL if user has not passed from outside.
 * @property eb_paymentSummaryDetails.proceedToPaymentUrl
 * @type {String}
 */
eb_paymentSummaryDetails.proceedToPaymentUrl = eb_paymentSummaryDetails.SitePath + "Checkout.html";

/**
 * Skipping payment step on checkout wizard if Order total is Zero.
 * @property eb_paymentSummaryDetails.orderConfirmationUrl
 * @type {String}
 */
eb_paymentSummaryDetails.orderConfirmationUrl = eb_paymentSummaryDetails.SitePath + "OrderConfirmation.html";

/*Success Response */
eb_paymentSummaryDetails.successResponses = {
 ...
 * 
 * @property eb_paymentSummaryDetails.errorResponses
 * @type {Object}
 * */
 ...
 * @property eb_paymentSummaryDetails.defaultErrorMessage
 * @type {String}
 * */
eb_paymentSummaryDetails.defaultErrorMessage = 'Sorry, there was an unexpected error. If the problem persists, please contact customer support for further assistance.';

/* Input Validations */
eb_paymentSummaryDetails.couponValidations = {
 ...
 * @method eb_paymentSummaryDetails.render
 ...
 * */
eb_paymentSummaryDetails.render = function (options) {
    ...
    if (options) {
        ...
        if (!options.templatePath) {
            options.templatePath = eb_paymentSummaryDetails.SitePath + eb_paymentSummaryDetails.TemplatePath;
        }
 ...
 * @method eb_paymentSummaryDetails.couponId
 * @param {String} couponName CoupanName
 * @return {Object} Return Id of Coupon. If not available then return -1
 * */
eb_paymentSummaryDetails.couponId = function (couponName) {
    var defer = eBusinessJQObject.Deferred();
    var service = eb_paymentSummaryDetails.cName;
    if (couponName) {
        service = eb_paymentSummaryDetails.cName + "/" + couponName;
    }
 ...
 * @method eb_paymentSummaryDetails.model
 ...
 */
eb_paymentSummaryDetails.model = function (options) {
    var _that = this;
    if (!options) {
    ...
    if (options.shoppingCart) {
        ...
        eb_paymentSummaryDetails.shoppingCart(options.shoppingCart);
    }

    eb_paymentSummaryDetails.domElement(_that.domElement);
    ...
    if (options.proceedToCheckoutUrl) {
        eb_paymentSummaryDetails.proceedToCheckoutUrl = options.proceedToCheckoutUrl;
    }

    /* Redirect to the billing and shipping page. */
    if (options.proceedToPaymentUrl) {
        eb_paymentSummaryDetails.proceedToPaymentUrl = options.proceedToPaymentUrl;
    }
    ...
    _that.checkCouponApplied = function (couponID, couponName, shoppingCartItems) {
        ...
        if (!shoppingCartItems) {
            if (eb_paymentSummaryDetails.viewCartObject) {
                eb_paymentSummaryDetails.shoppingCart.viewCart().done(function (result) {
                    if (eb_paymentSummaryDetails.viewCartObject.getSubscriptionAndEventCartItems) {
                        eb_paymentSummaryDetails.viewCartObject.getSubscriptionAndEventCartItems().done(function (data) {
                            _that.updateCouponAppliedMessage(couponID, couponName, eb_paymentSummaryDetails.shoppingCart.cartItems());
    ...
    var relatedProductsWidget = eBusinessJQObject("#relatedProducts");

    if (relatedProductsWidget.length) {
        /* The Summary Widget at the right side of the page will be fixed on scrolling until the related products widget section */
        winObject.scroll(function () {
            var relatedProducts = relatedProductsWidget.offset();
            var paymentPanel = eBusinessJQObject("#paymentPanel");
            paymentPanel.removeAttr("style");

            if (winObject.scrollTop() >= 50 && winObject.height() >= 535 && winObject.scrollTop() + winObject.height() <= relatedProducts.top) {
                summaryStartScrolling();
            } else {
                if (winObject.scrollTop() + winObject.height() > relatedProducts.top && relatedProducts.top - 615 > 0) {
                    _that.fixedPanelev("fixedPanelpaymentWithRelatedProducts");
                    eBusinessJQObject(".fixedPanelpaymentWithRelatedProducts").css({
                        top: relatedProducts.top - 615
                    });
                }
                else {
    ...
    _that.updateCouponAppliedMessage = function (couponID, couponName, shoppingCartItems) {
        ...
        if (parseFloat(totalSavings) <= 0) {
            _that.couponValidationMessage(eb_paymentSummaryDetails.couponValidations['Coupon not applicable']);
            _that.couponClass('couponFailed');
            _that.disableTextCoupon(0);
        }
        else {
            _that.couponValidationMessage(eb_paymentSummaryDetails.successResponses['Coupon applied.']);
    ...
    _that.applyCoupon = function () {
        if (_that.couponName() !== '') {
            _that.couponNameCheck = function () {
                ...
                return eb_paymentSummaryDetails.couponId(_that.couponName());
            };
                ...
                if (data.id > 0) {
                    if (_that.shoppingCart) {
                        ...
                        _that.shoppingCart.updateShoppingCart(couponid).done(function (result) {
                            ...
                            _that.couponValidation(1);
                            _that.couponValidationMessage(eb_paymentSummaryDetails.successResponses['Coupon not valid']);
                            _that.couponClass('couponFailed');
            ...
            _that.couponValidation(1);
            _that.couponValidationMessage(eb_paymentSummaryDetails.couponValidations['Enter coupon']);
            _that.couponClass('couponFailed');
        }
    };
    ...
    _that.proceedToCheckout = function () {
        if (_that.userContext.isUserLoggedIn()) {
            if (eb_paymentSummaryDetails.proceedToCheckoutUrl) {
                window.location.assign(eb_paymentSummaryDetails.proceedToCheckoutUrl);
            }
            else {
    ...
    _that.proceedToPayment = function (val) {
        if (val && val.total() === 0) {
            ...
            _that.shoppingCart.placeOrderForZeroDollarProduct().done(function (data) {
                if (eb_paymentSummaryDetails.orderConfirmationUrl) {
                    window.location.assign(eb_paymentSummaryDetails.orderConfirmationUrl + "?" + encodeURIComponent("orderId") + "=" + encodeURIComponent(data.id)); /* Updates all our instances of encodeURIComponent. */
                }
                else {
                ...
                if (xhr && typeof xhr.responseJSON !== 'undefined')
                    _that.errorMessage(eb_Config.getErrorMessageForControl(xhr.responseJSON, eb_paymentSummaryDetails));
                else
                    _that.errorMessage(eb_paymentSummaryDetails.defaultErrorMessage);
            });
        }
        else {
            if (eb_paymentSummaryDetails.proceedToPaymentUrl) {
                window.location.assign(eb_paymentSummaryDetails.proceedToPaymentUrl);
            }
            else {
 ...
 * @method eb_paymentSummaryDetails.shoppingCart
 * @param {Object} shoppingCart  shopping cart object.
 * */
eb_paymentSummaryDetails.shoppingCart = function (shoppingCart) {
    var self = this;
...
/**
 * View cart Object
 * @method eb_paymentSummaryDetails.viewCartObject
 * @param {Object} viewCartObject  viewCart object.
 * */
eb_paymentSummaryDetails.viewCartObject = function (viewCartObject) {
    var self = this;
...
/**
* Page DOM element.
* @method eb_paymentSummaryDetails.domElement
* @param {object} domElement current DOM element.
* */
eb_paymentSummaryDetails.domElement = function (domElement) {
    var self = this;
