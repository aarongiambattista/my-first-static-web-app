 ...
 * @class eb_adminViewCart
 * */
var eb_adminViewCart = eb_adminViewCart || {};

/**
 * Control level setting: Site path.
 * @property eb_adminViewCart.SitePath
 * @type {String}
 */
eb_adminViewCart.SitePath = eb_Config.SitePath;
eb_adminViewCart.adminSitePath = eb_Config.adminSitePath;

/**
 * Control level setting Template path.
 * @property eb_adminViewCart.TemplatePath
 * @type {String}
 */
eb_adminViewCart.TemplatePath = "admin/ViewCart.html";

/**
 * The path to the eBusiness SOA layer.
 * @property eb_adminViewCart.ServicePath
 * @type {String}
 */
eb_adminViewCart.ServicePath = eb_Config.ServicePathV1;

/**
 * Redirect to product Details Page.
 * @property eb_adminViewCart.productDetailsURL
 * @type {String}
 */
eb_adminViewCart.productDetailsURL = eb_adminViewCart.adminSitePath + "ProductDetails.html";

/**
 * Redirect to event Details Page.
 * @property eb_adminViewCart.eventDetailsURL
 * @type {String}
 */
eb_adminViewCart.eventDetailsURL = eb_adminViewCart.adminSitePath + "EventDetails.html";

/**
 * Redirect to event registration Page.
 * @property eb_adminViewCart.eventRegistrationURL
 * @type {String}
 */
eb_adminViewCart.eventRegistrationURL = eb_adminViewCart.adminSitePath + "EventRegistration.html";

/**
 * Default product image.
 * @property eb_adminViewCart.defaultImage
 * @type {String}
 */
eb_adminViewCart.defaultImage = eb_Config.SitePath + "/images/products/coming-soon.png";

/*Success Response */
eb_adminViewCart.successResponses = {
 ...
 * 
 * @property eb_adminViewCart.errorResponses
    * @type { Object }
 **/
 ...
 * @property eb_adminViewCart.defaultErrorMessage
 * @type {String}
 * */
eb_adminViewCart.defaultErrorMessage = 'Sorry, there was an unexpected error. If the problem persists, please contact customer support for further assistance.';

/* Default message when cart is empty */
eb_adminViewCart.cartEmptyMessage = "Your Cart is Empty";
 ...
 * @method eb_adminViewCart.render
 ...
 * */
eb_adminViewCart.render = function (options) {
    ...
    if (options) {
        ...
        if (!options.templatePath) {
            options.templatePath = eb_adminViewCart.SitePath + eb_adminViewCart.TemplatePath;
        }
 ...
 */
eb_adminViewCart.model = function (options) {
    var _that = this;
    ...
    if (options.shoppingCart) {
        eb_adminViewCart.shoppingCart(options.shoppingCart);
    }
    ...
    eb_adminViewCart.domElement(_that.domElement);
    ko.validation.init({
    ...
    _that.viewCartItems = function (data) {
        ...
        if (eb_Config.loadDefaultImage) {
            self.productImage = ko.observable(eb_adminViewCart.defaultImage);
        }
        else {
        ...
        */
        ...
        if (eb_adminViewCart.isOrderHistory) {
        ...
        self.autoRenewCheck = function (data) {
            if (eb_adminViewCart.shoppingCart) {
                var dataToUpdate = {};
                ...
                eb_adminViewCart.shoppingCart.updateCartItem(dataToUpdate).done(function (result) {
                    _that.showError(0);
                    if (Array.isArray(result)) { result = result[0]; }
                    self.autoRenew(result.autoRenew);
                }).fail(function (data, msg, jhr) {
                    _that.showError(1);
                    _that.errorMessage(eb_adminViewCart.defaultErrorMessage);
    ...
    _that.productNameDetails = function (item) {
        if (eb_adminViewCart.productDetailsURL || eb_adminViewCart.eventDetailsURL) {
            if (item.productType().toLowerCase() === "meeting") {
                window.location.assign(eb_adminViewCart.eventDetailsURL + "?" + encodeURIComponent("productId") + "=" + encodeURIComponent(item.productId()));
            } else {
                window.location.assign(eb_adminViewCart.productDetailsURL + "?" + encodeURIComponent("productId") + "=" + encodeURIComponent(item.productId()));
            }
        }
        else {
    ...
    _that.loadViewCartItems = function (data) {
        ...
        * if we don't clear this, it loads items multiple times */
        ...
        if (data.length === 0) {
            ...
            _that.cartEmptyMessage(eb_adminViewCart.cartEmptyMessage);
        }
    ...
    _that.calculateTotalSavings = function (cartItems) {
        ...
        eBusinessJQObject.each(cartItems || eb_adminViewCart.shoppingCart.cartItems(), function (idx, cartItem) {
    ...
    if (_that.data) {
        ...
        if (eb_adminViewCart.shoppingCart) {
            /* showing total savings */
            _that.totalSavings(0);
            _that.loadViewCartPaymentDetails(eb_adminViewCart.shoppingCart);
            _that.loadViewCartItems(eb_adminViewCart.shoppingCart.cartItems());
        }
    }
    ...
    _that.removeProduct = function (data) {
        if (eb_adminViewCart.shoppingCart) {
            ...
            if (data.productType().toLowerCase() !== "meeting") {
                eb_adminViewCart.shoppingCart.removeCartItem(dataToRemove).done(function (result) {
                    _that.showError(0);
                ...
                eb_adminViewCart.shoppingCart.removeEventItems(dataToRemove).done(function (result) {
                    _that.showError(0);
                    loadSubscriptionAndEventCartItems();
                }).fail(function (data, msg, jhr) {
                    _that.showError(1);
    ...
    _that.getSubscriptionAndEventCartItems = function () {
        var def = eBusinessJQObject.Deferred();
        eBusinessJQObject.when(eb_adminShoppingCart.viewSubscriptionCartItems(eb_adminViewCart.shoppingCart.cartItems()),
            eb_adminShoppingCart.getEventProductItems(eb_adminViewCart.shoppingCart.cartItems())).done(function (subscriptionAllItems, eventAllItems) {
    ...
    function callShowCartUpdate() {
        var showCartUpdate = eBusinessJQObject(eb_adminViewCart.domElement).find('#CartUpdateMsg')[0];
                ...
                if (eb_adminViewCart.shoppingCart) {
                    ...
                    eb_adminViewCart.shoppingCart.updateCartItem(dataToUpdate).done(function (result) {
                        ...
                        if (data1 && typeof data1.responseJSON !== 'undefined') {
                            _that.errorMessage(eb_Config.getErrorMessageForControl(data1.responseJSON, eb_adminViewCart));
                        } else {
                            _that.errorMessage(eb_adminViewCart.defaultErrorMessage);
                        }
                    });
    ...
    _that.editMeeting = function (item) {
        if (eb_adminViewCart.eventRegistrationURL) {
            if (item.productType().toLowerCase() === "meeting") {
                window.location.assign(eb_adminViewCart.eventRegistrationURL + "?" + encodeURIComponent("productId") + "=" + encodeURIComponent(item.productId()) + "&hasSessions=" + _that.hasSessions());
            }
        }
        else {
            console.error("A redirection page is missing.");
    ...
    _that.removeAllProduct = function () {
        if (eb_adminViewCart.shoppingCart) {
            eb_adminViewCart.shoppingCart.emptyCart().done(function () {
                _that.showError(0);
                _that.loadViewCartPaymentDetails(eb_adminViewCart.shoppingCart);
                _that.viewCartProduct.removeAll();
                _that.removeAllDetailsVisible(0);
 ...
 * @method eb_adminViewCart.shoppingCart
 * @param {Object} shoppingCart  shopping cart object.
 * */
eb_adminViewCart.shoppingCart = function (shoppingCart) {
    var self = this;
...
ko.bindingHandlers.imageSrc = {
    update: function (element, valueAccessor) {
        ...
        eBusinessJQObject('<img />').attr('src', src).on('load', function () {
            ...
            eBusinessJQObject(element).attr('src', eb_adminViewCart.defaultImage);
        });
    }
};
...
/**
* Page DOM element.
* @method eb_adminViewCart.domElement
* @param {object} domElement current DOM element.
* */
eb_adminViewCart.domElement = function (domElement) {
    var self = this;
...
eBusinessJQObject(eb_adminViewCart.domElement).ajaxError(function (event, xhr, settings) {
    if (xhr && typeof xhr.responseJSON !== 'undefined' && (xhr.responseJSON.type === eb_Config.serviceErrorType['Security Requirement Failed'] || xhr.responseJSON.type === eb_Config.serviceErrorType['Http Response']))
        eb_Config.getErrorMessageForControl(xhr.responseJSON, eb_adminViewCart);
});