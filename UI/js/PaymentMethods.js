 ...
 * @class eb_paymentControl
 * */
var eb_paymentControl = eb_paymentControl || {};

/**
 * Control level setting: Site path.
 * @property eb_paymentControl.SitePath
 * @type {String}
 */
eb_paymentControl.SitePath = eb_Config.SitePath;

/**
 * The path to the eBusiness SOA layer.
 * @property eb_paymentControl.ServicePath
 * @type {String}
 */
eb_paymentControl.ServicePath = eb_Config.ServicePathV1;

/**
 * Control level setting Template path.
 * @property eb_paymentControl.TemplatePath
 * @type {String}
 */
eb_paymentControl.TemplatePath = "html/PaymentMethods.html";

/**
 * GET service to get all valid payment types.
 * @property eb_paymentControl.getValidPaymentTypes
 * @type {String}
 */
eb_paymentControl.getValidPaymentURL = eb_paymentControl.ServicePath + "ProfilePersons/{personId}/ValidPayments";

/**
 * GET service to get all Saved Cards records.
 * @property eb_paymentControl.getSavedCardsService
 * @type {String}
 */
eb_paymentControl.getSavedCardsService = eb_paymentControl.ServicePath + "ProfilePersons/{personId}/SavedPaymentMethods/CreditCard";

/**
 * Service URL to update/edit Saved Cards records.
 * @property eb_paymentControl.editSavedPaymentURL
 * @type {String}
 */
eb_paymentControl.editSavedPaymentURL = eb_paymentControl.ServicePath + 'ProfilePersons/{person ID}/SavedPaymentMethods/CreditCard/{card ID}';

/**
 * Service URL to delete Saved Cards records.
 * @property eb_paymentControl.deleteSavedPaymentURL
 * @type {String}
 */
eb_paymentControl.deleteSavedPaymentURL = eb_paymentControl.ServicePath + 'ProfilePersons/{person ID}/SavedPaymentMethods/{card ID}';

/**
 * Service URL to get Saved Cards records for ACH.
 * @property eb_paymentControl.getSavedCardForAch
 * @type {String}
 */
eb_paymentControl.getSavedCardForAch = eb_paymentControl.ServicePath + 'ProfilePersons/{personId}/SavedPaymentMethods/ACH';

/**
 * PATCH ACH record.
 * @property eb_paymentControl.updateACHrecordService
 * @type {string}
 * */
eb_paymentControl.updateACHrecordService = eb_paymentControl.ServicePath + "ProfilePersons/{personId}/SavedPaymentMethods/ACH/{savedPaymentId}"

/**Payment methods for companies */

/**
 * GET service to get all valid payment types for a company.
 * @property eb_paymentControl.getValidPaymentURLCompany
 * @type {String}
 */
eb_paymentControl.getValidPaymentURLCompany = eb_paymentControl.ServicePath + "admin/company/{id}/ValidPayments";

/**
 * GET service to get all Saved Cards records for a company.
 * @property eb_paymentControl.getSavedCardsServiceCompany
 * @type {String}
 */
eb_paymentControl.getSavedCardsServiceCompany = eb_paymentControl.ServicePath + "admin/company/{id}/SavedPaymentMethods/CreditCard";

/**
 * Service URL to get Saved Cards records for ACH for a company.
 * @property eb_paymentControl.getSavedCardForAchCompany
 * @type {String}
 */
eb_paymentControl.getSavedCardForAchCompany = eb_paymentControl.ServicePath + 'admin/company/{id}/SavedPaymentMethods/ACH';


/**
 * Object with each month's numeric value.
 * @property: eb_paymentControl.monthConstants
 * @type {Object}
 * */
...
/* Success Responses */
eb_paymentControl.successResponses = {
...
/* Error messages */
eb_paymentControl.errorMessages = {
 ...
 * 
 * @property eb_paymentControl.errorResponses
 * @type {Object}
 * */
 ...
 * @property eb_paymentControl.defaultErrorMessage
 * @type {String}
 * */
eb_paymentControl.defaultErrorMessage = 'Sorry, there was an unexpected error. If the problem persists, please contact customer support for further assistance.';

/**
 * All card types.
 * @property eb_paymentControl.creditCardtypes
 * @type {Object}
 * */
eb_paymentControl.creditCardtypes = ["Visa", "Mastercard", "American Express", "Discover"];

/**
 * All active payment types
 * @property: eb_paymentControl.activePaymentTypes
 * @type {Object}
 */
...
/**
 * All active payment types
 * @property: eb_paymentControl.validPaymentTypesForUser
 * @type {Object}
 */
 ...
 * @method eb_paymentControl.render
 ...
 * */
eb_paymentControl.render = function (options) {
    ...
    if (options) {
        ...
        if (!options.templatePath) {
            var finalPath = eb_paymentControl.SitePath + eb_paymentControl.TemplatePath;
 ...
 * @method eb_paymentControl.getAllACHSavedCardRecords
 * @param {Number} personId Person ID.
 * @return {Object} Returns jquery promise which resolves to list of saved cards.
 * */
eb_paymentControl.getAllACHSavedCardRecords = function (personId) {
    ...
    eBusinessJQObject.get(
        {
            url: eb_paymentControl.getSavedCardForAch.replace("{personId}", personId),
            xhrFields: {
 ...
 * @method eb_paymentControl.updateACHCardRecord
 ...
 */
eb_paymentControl.updateACHCardRecord = function (data, personId, cardId) {
    var def = eBusinessJQObject.Deferred();
    if (!data) {
    ...
    eb_Config.retrieveCSRFTokens().always(function (headers) {
        eBusinessJQObject.ajax({
            url: eb_paymentControl.updateACHrecordService.replace("{personId}", personId).replace("{savedPaymentId}", cardId),
            type: "PATCH",
            data: data,
            xhrFields: {
                withCredentials: true
 ...
 * @method eb_paymentControl.getAllACHSavedCardRecordsCompany
 * @param {Number} companyId Company ID.
 * @return {Object} Returns jquery promise which resolves to list of saved cards.
 * */
eb_paymentControl.getAllACHSavedCardRecordsCompany = function (companyId) {
    var deferred = eBusinessJQObject.Deferred();
    ...
    eBusinessJQObject.get(
        {
            url: eb_paymentControl.getSavedCardForAchCompany.replace("{id}", companyId),
            xhrFields: {
                withCredentials: true
            }
        }
 ...
 * @method eb_paymentControl.paymentModel
 ...
 */
eb_paymentControl.paymentModel = function (options, control) {
    var _that = this;
    ...
    _that.showPaymentMethodsForUser = function () {
        ...
        for (var i = 0; i < _that.validPayments.length; i++) {
            ...
            /* PO Section */
            if (_that.validPayments[i].name === eb_paymentControl.validPaymentTypesForUser['Purchase Order'])
                _that.showBillMeLaterPanel(1);

            /* ACH Section */
            if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser["Wire Transfer"])
                _that.showACHPanel(1);

            /* Credit Card Section */
            if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser['Credit Card'] || _that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser['Credit Card Reference Transaction']) {
                if (!_that.showHostedPaymentPanel() && !_that.showiFrameTokenizerPaymentPanel())
                    _that.showCreditCards(1);
            }

            /* Hosted Payment Section */
            if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser['Credit Card Hosted Payment Reference Transaction'] && _that.validPayments[i].IsRemote === true) {
            ...
            if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser['Hosted iFrame Tokenizer']) {
                _that.showCreditCards(0);
           ...
           /* Google Pay Section */
            if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser["Google Pay"])
                _that.showGPayPanel(1);

            /*Apple Pay Section*/
            if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser["Apple Pay"])
    ...
    //    for (var i = 0; i < _that.validPayments.length; i++) {
    //        if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser['Credit Card']
    //            || _that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser['Credit Card Reference Transaction']) {
    ...
    //    for (var i = 0; i < _that.validPayments.length; i++) {
    //        if (_that.validPayments[i].PaymentType === eb_paymentControl.validPaymentTypesForUser['Credit Card Hosted Payment Reference Transaction']
    ...
    _that.selectedMonthNumber = ko.observable(eb_paymentControl.monthConstants[_that.selectedMonth()]);
    ...
    _that.achListOfCards = function (data) {
        ...
        self.editACHCard = function (data) {
            ...
            _that.showErrorMessage(0);
            eb_paymentControl.updateACHCardRecord(dataToUpdate, _that.parentControl.personId, data.id).done(function (result) {
                ...
                if (spmRecord) {
                    ...
                    _that.successMessage(eb_paymentControl.successResponses['Card updated']);
                }
            }).fail(function (xhr, textStatus, errorThrow) {
                _that.showErrorMessage(1);
                _that.showCardSuccess(0);
                if (xhr && typeof xhr.responseJSON !== 'undefined')
                    _that.errorMessage(eb_Config.getErrorMessageForControl(xhr.responseJSON, eb_paymentControl));
                else
                    _that.errorMessage(eb_paymentControl.errorMessages['Update card failed']);
            });
        }
        ...
        self.deleteCardDetails = function (cardInfo) {
            var param = {};
            param.serviceURL = eb_paymentControl.deleteSavedPaymentURL.replace('{person ID}', _that.parentControl.personId).replace('{card ID}', cardInfo.id);
            _that.showLoader(1);
            eb_paymentControl.deleteSavedCard(param).done(function () {
                _that.reloadSavedCard().done(function () {
                    ...
                    if (xhr && typeof xhr.responseJSON !== 'undefined')
                        ...
                        _that.errorMessage(eb_paymentControl.defaultErrorMessage);
                });
            }).fail(function (xhr, msg, data) {
                _that.showSuccessMessage(0);
                _that.showErrorMessage(1);
    ...
    _that.reloadSavedCard = function () {
        var def = eBusinessJQObject.Deferred();
        eBusinessJQObject.when(eb_paymentControl.getAllRecords(_that.parentControl.personId),
            eb_paymentControl.getAllACHSavedCardRecords(_that.parentControl.personId)).done(function (savedCardList, achCardList) {
                _that.savedCardList('');
    ...
    _that.reloadSavedCardCompany = function () {
        var def = eBusinessJQObject.Deferred();
        eBusinessJQObject.when(eb_paymentControl.getAllRecordsCompany(_that.parentControl.companyId),
            eb_paymentControl.getAllACHSavedCardRecordsCompany(_that.parentControl.companyId)).done(function (savedCardList, achCardList) {
                _that.savedCardList('');
                _that.savedCardsCollection('');
                _that.savedCardForACH.removeAll();
                /*Check for SavedCard*/
    ...
    _that.savedCardModel = function (data) {
        ...
        self.updateCardDetails = function (cardInfo) {
            var cardDetails = {
                expirationMonth: eb_paymentControl.monthConstants[cardInfo.newMonth()],
                expirationYear: cardInfo.newYear()
            };

            var param = {};
            param.serviceURL = eb_paymentControl.editSavedPaymentURL.replace('{person ID}', _that.parentControl.personId).replace('{card ID}', cardInfo.id());
            param.cardData = cardDetails;

            eb_paymentControl.editSavedCard(param).done(function (data) {
                _that.showErrorMessage(0);
                _that.showSuccessMessage(1);
                _that.successMessage(eb_paymentControl.successResponses['Card updated']);
        ...
        self.deleteCardDetails = function (cardInfo) {
            ...
            eb_paymentControl.deleteSavedCard(param).done(function () {
                eb_paymentControl.getAllRecords(_that.parentControl.personId).done(function (savedCardList) {
                    ...
                    _that.showErrorMessage(0);
                    _that.showSuccessMessage(1);
                    _that.successMessage(eb_paymentControl.successResponses['Card deleted']);
                }).fail(function (xhr, msg, data) {
                    _that.showSuccessMessage(0);
                    _that.showErrorMessage(1);
                    if (xhr && typeof xhr.responseJSON !== 'undefined')
    ...
    _that.loadCreditCardImages = function () {
        for (var i = 0; i < 4; i++) {
            _that.creditCardImageList.push(new _that.creditCardImagesModel(eb_paymentControl.creditCardtypes[i]));
        }
    };
 ...
 * @method eb_paymentControl.getValidPayments
 * @param {Number} personId Person ID.
 * @return {Object} Returns jquery promise which resolves to list of valid payments.
 * */
eb_paymentControl.getValidPayments = function (personId) {
    var deferred = eBusinessJQObject.Deferred();
    if (!personId || personId <= 0) {
        throw { type: "argument_null", message: "personId property is required.", stack: Error().stack };
    }
    eBusinessJQObject.get(
        {
            url: eb_paymentControl.getValidPaymentURL.replace("{personId}", personId),
            xhrFields: {
                withCredentials: true
            }
        }
 ...
 * @method eb_paymentControl.getAllRecords
 * @param {Number} personId Person ID.
 * @return {Object} Returns jquery promise which resolves to list of saved cards.
 * */
eb_paymentControl.getAllRecords = function (personId) {
    var deferred = eBusinessJQObject.Deferred();
    if (!personId || personId <= 0) {
        throw { type: "argument_null", message: "personId property is required.", stack: Error().stack };
    }
    eBusinessJQObject.get(
        {
            url: eb_paymentControl.getSavedCardsService.replace("{personId}", personId),
            xhrFields: {
                withCredentials: true
            }
        }
 ...
 * @method eb_paymentControl.editSavedCard
 * @param {Number} cardDetails Saved Cards Details.
 * @return {Object} Returns jquery promise.
 * */
eb_paymentControl.editSavedCard = function (cardDetails) {
    var deferred = eBusinessJQObject.Deferred();
    var _that = this;
 ...
 * @method eb_paymentControl.deleteSavedCard
 * @param {Number} cardDetails Saved Cards Details.
 * @return {Object} Returns jquery promise.
 * */
eb_paymentControl.deleteSavedCard = function (cardDetails) {
    var deferred = eBusinessJQObject.Deferred();
    var _that = this;
    _that.serviceURL = cardDetails.serviceURL;
    eb_Config.retrieveCSRFTokens().always(function (headers) {
 ...
 * @method eb_paymentControl.getValidPaymentsCompany
 * @param {Number} companyId company ID.
 * @return {Object} Returns jquery promise which resolves to list of valid payments.
 * */
eb_paymentControl.getValidPaymentsCompany = function (companyId) {
    var deferred = eBusinessJQObject.Deferred();
    if (!companyId || companyId <= 0) {
        throw { type: "argument_null", message: "companyId property is required.", stack: Error().stack };
    }
    eBusinessJQObject.get(
        {
            url: eb_paymentControl.getValidPaymentURLCompany.replace("{id}", companyId),
            xhrFields: {
                withCredentials: true
            }
        }
 ...
 * @method eb_paymentControl.getAllRecordsCompany
 * @param {Number} companyId company ID.
 * @return {Object} Returns jquery promise which resolves to list of saved cards.
 * */
eb_paymentControl.getAllRecordsCompany = function (companyId) {
    var deferred = eBusinessJQObject.Deferred();
    if (!companyId || companyId <= 0) {
        throw { type: "argument_null", message: "companyId property is required.", stack: Error().stack };
    }
    eBusinessJQObject.get(
        {
            url: eb_paymentControl.getSavedCardsServiceCompany.replace("{id}", companyId),
            xhrFields: {
                withCredentials: true
            }
        }
